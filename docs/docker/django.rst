...
.. note::
   ...
   The ``gthreads`` worker class is synchronous, `despite <https://github.com/benoitc/gunicorn/issues/1493#issuecomment-321463257>`__ appearing under `AsyncIO Workers <https://docs.gunicorn.org/en/stable/design.html#asyncio-workers>`__.
...
When using the ``gthreads`` worker class, a main thread `handles the heartbeat <https://docs.gunicorn.org/en/stable/design.html#how-many-threads>`__. As such, we use the ``gthreads`` worker class, to not have to worry about heartbeat timeouts.
