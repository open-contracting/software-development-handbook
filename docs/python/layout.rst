...
-  If a module defines names that are imported by only one other module, merge the modules unless:
   ...
   - It is a convention of a framework (for example, the files created by Django's `startapp <https://docs.djangoproject.com/en/stable/intro/tutorial01/#creating-the-polls-app>`__ command).
...
-  Put `tests outside application code <https://docs.pytest.org/en/latest/explanation/goodpractices.html#choosing-a-test-layout>`__. Do not add ``tests`` directories inside application code.
